...
The latest bounded live refresh completed at `2026-09-12T10:06:57.549Z` UTC (`06:06:57 EDT`). Its compact capture is in [`data/seed/observations.json`](../../data/seed/observations.json). Raw HTML is retained locally under `data/raw/` (ignored by Git), including the timestamped CMU, Lobos index, Lobos Bentley detail, and Reinhold responses.
...
| --- | ---: | --- |
| CMU Off-Campus Housing | 26 | 10 card observations plus 16 nearby active 2-bedroom floorplans from embedded `listingData`; exact bed/bath, rent or range, optional availability, coordinates, utility labels, lease labels, amenities, and a source photo when present |
...
VALID 41 homes, 420 evidence rows, 3 imported source runs
```
