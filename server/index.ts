...
const { app } = createApp({ store, workflows: createWorkflows(store, { useResearchWorker: process.env.ADDRESS_RESEARCH_WORKER !== 'off' }), staticDirectory: existsSync(staticDirectory) ? staticDirectory : undefined });
const server = app.listen(port, '127.0.0.1', () => process.stdout.write(`Address is ready at http://127.0.0.1:${port}\n`));
