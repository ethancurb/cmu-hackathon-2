...
export async function findDestinations(query: string, market: Criteria['market'], signal: AbortSignal, options: GeocodeOptions = {}): Promise<Destination[]> {
  ...
  try { places = JSON.parse(await readFile(cacheFile, 'utf8')) as NominatimPlace[]; } catch {
    ...
    places = candidate as NominatimPlace[];
    try { await mkdir(cacheDirectory, { recursive: true }); await writeFile(cacheFile, JSON.stringify(places), 'utf8'); } catch { /* cache is optional */ }
  }
  return places.flatMap((place): Destination[] => {
    ...
    return [destinationFrom(place.display_name, coordinate, `nominatim:${place.place_id}`, 'Geocoded only after explicit submit; verify the mapped entrance before relying on route qualification.')];
  });
}

export function geocodeEvidence(destination: Destination, observedAt: string): { source: SourceEntry; evidence: Evidence } {
